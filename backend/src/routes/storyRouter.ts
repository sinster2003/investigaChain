...
storyRouter.get("/getallstories", async (req: Request, res: Response) => {
    try {
...
storyRouter.post("/uploadstory", authMiddleware, metamaskMiddleware, async (req: metamaskType, res: Response) => {
    ...
    try {
        ...
        try {
            ...
            const stopwordsRemovedWords = removeStopwords(tokenizedWords);

            const TfIdf = natural.TfIdf;
            const tfidf = new TfIdf();
            tfidf.addDocument(stopwordsRemovedWords);
            const terms = tfidf.listTerms(0);

            const patternizedWords = terms.sort((a, b) => b.tfidf - a.tfidf);

            const patterns = patternizedWords.slice(0, 20).map((word) => word.term);
...
storyRouter.get("/getstory/:storyid", async (req: Request, res: Response) => {
...
storyRouter.get("/getfilters", async (req: Request, res: Response) => {
